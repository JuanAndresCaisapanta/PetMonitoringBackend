...
import com.MonitoreoMascotas.Dto.VacunasDto;
import com.MonitoreoMascotas.Entity.Vacunas;
import com.MonitoreoMascotas.Service.VacunasService;
...
public class VacunasController {
	...
	public ResponseEntity<List<Vacunas>> lista() {
		List<Vacunas> list = vacunasService.lista();
		return new ResponseEntity<>(list, HttpStatus.OK);
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	@GetMapping("/listaid/{id}")
	public ResponseEntity<Vacunas> getById(@PathVariable("id") int id) {
		if (!vacunasService.existsById(id))
			return new ResponseEntity(new Mensaje("no existe"), HttpStatus.NOT_FOUND);
		Vacunas vacuna = vacunasService.getOne(id).get();
		return new ResponseEntity<>(vacuna, HttpStatus.OK);
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	@GetMapping("/listanombre/{nombre}")
	public ResponseEntity<Vacunas> getByNombre(@PathVariable("nombre") String nombre) {
		if (!vacunasService.existsByNombre(nombre))
			return new ResponseEntity(new Mensaje("no existe"), HttpStatus.NOT_FOUND);
		Vacunas vacuna = vacunasService.getByNombre(nombre).get();
		return new ResponseEntity<>(vacuna, HttpStatus.OK);
	}

	@PostMapping(produces = "application/json")
	public ResponseEntity<?> crear(@RequestBody @Validated VacunasDto vacunasDto) throws ParseException {
		if (StringUtils.isBlank(vacunasDto.getFabricador()))
		...
		vacunasService.guardar(vacuna);
		return new ResponseEntity<>(new Mensaje("Vacuna creada"), HttpStatus.OK);
	}

	@PutMapping("{id}")
	public ResponseEntity<?> actualizar(@PathVariable("id") int id, @RequestBody VacunasDto vacunasDto) throws ParseException {
		if (!vacunasService.existsById(id))
			return new ResponseEntity<>(new Mensaje("no existe"), HttpStatus.NOT_FOUND);
		if (StringUtils.isBlank(vacunasDto.getFabricador()))
			return new ResponseEntity<>(new Mensaje("el nombre es obligatorio"), HttpStatus.BAD_REQUEST);
		...
		vacuna.setFechaRevacuna(vacunasDto.getFechaRevacuna());
		vacuna.setMascotas(vacunasDto.getMascotas());
		vacunasService.actualizar(vacuna);
		return new ResponseEntity<>(new Mensaje("Vacuna actualizada"), HttpStatus.OK);
	}
