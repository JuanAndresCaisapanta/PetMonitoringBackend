...
public interface EstablishmentRepository extends CrudRepository<Establishment, Long> {
	...
	@Query(value = "SELECT e FROM Establishment e, Pet p, Users u WHERE e.pet.id = p.id and p.users.id =u.id and u.status =true ORDER BY e.id  asc")
	List<Establishment> findEstablishmentByOrderByIdAsc();
